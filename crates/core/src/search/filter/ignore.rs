...
pub fn build_gitignore_matcher(
...
) -> Result<Option<Gitignore>, FilterError> {
    ...
    if ignore_config
        .sources
        .contains(super::config::IgnoreSources::GLOBAL)
    ...
    if ignore_config
        .sources
        .contains(super::config::IgnoreSources::PARENT)
    {
        let mut dir = root.parent();
        while let Some(parent) = dir {
            if ignore_config
                .sources
                .contains(super::config::IgnoreSources::VCS)
            {
            ...
            if ignore_config
                .sources
                .contains(super::config::IgnoreSources::DOT)
            {
        ...
        .contains(super::config::IgnoreSources::VCS)
    {
        ...
        if gitignore_path.is_file() && (!ignore_config.require_git || root.join(".git").is_dir()) {
    ...
    if ignore_config
        .sources
        .contains(super::config::IgnoreSources::EXCLUDE)
    {
