...
- `QuerySpec`: neutral query description (patterns, flags). Built by `Query::query_spec()` in `grep/search/query.rs`.
...
- Put regex matching or hit collection in this module — that belongs in `grep/search/run.rs`.
