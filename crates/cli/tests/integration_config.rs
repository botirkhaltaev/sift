...
use common::{
    TestProject, assert_exit_code, assert_stderr_empty, assert_stdout_contains, normalize_stderr,
};
...
#[test]
fn missing_config_path_warns_and_continues() {
    ...
    let out = cmd.args(["needle"]).output().unwrap();
    assert_stdout_contains(&out, "a.txt:needle");
