...
# Usage: ./scripts/bench.sh [core|query|index|grep|candidates|cli]
...
#   candidates — sift-core candidate planning benchmark
...
case "$target" in
  ...
  candidates) exec cargo bench -p sift-core --bench candidates "$@" ;;
  cli)        exec cargo bench -p sift-grep --bench cli "$@" ;;
  *)          echo "Usage: $0 [core|query|index|grep|candidates|cli]" >&2; exit 1 ;;
esac
